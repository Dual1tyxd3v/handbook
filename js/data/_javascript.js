...
12) <b>.at(x)</b> - позволяет получить элемент по заданному индексу х, который также может быть отрицательным числом. Удобен для доступа к последнему элементу<br>
...
4) указывает использовать ли разделители чисел (default = true)`
  ...
  [
    `async/await`,
    `Это синтаксический сахар который позволяет писать асинхронный код в виде синхронного. Сами async функции всегда возвращают промис независимо от того что переданно в return. Поэтому в случае если в функции возможна ошибка ее следует отловить с помощью try/catch и вернуть новый экземпляр ошибки`
  ],
  [
    `Собственные события`,
    `<pre>
...
return x - y;
  ...
  [
    ...
    `Метод объекта window позволяет прокрутить документ до координат х, у. z - объект настроек где можно например указать плавность прокрутки ({ behavior: 'smooth' })`
  ],
  [
    `.scrollIntoView()`,
