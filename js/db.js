...
class Another extends Some {
  constructor(a, b, c) {
    ...
    [
      `Что такое DOM`,
      `Объектная модель документа представляющая интерфейс повзоляющий взаимодействовать с разметкой страницы`
    ],
    [
      `.keyCode`,
    ...
    [
      `delete o.v`,
      `Метод позволяет удалить свойство v из объекта о`
    ],
    [
      `REST API`,
    ...
    [
      ...
      `n..toString() - для преобразования числа в строку ставят 2 точки потому что интерпритатор ожидает сначала увидеть число в виде n.m а потом уже видит метод`
    ],
    [
...
switch(x) {
  ...
  break;
    ...
    [
      ...
      `const ar2 = [].concat(ar1); - 1 способ<hr>const ar3 = [...ar1, ...ar2, 'some']; - 2 способ через спред оператор`
    ],
    [
      `Мемоизация`,
...
<b>Если говорить еще проще то по сути мы имеем какую то ф-цию обертку в которой объявляем переменные которые хотим изолировать а затем возвращаем какую то ф-цию с логикой которая производит какие то операции с нашей(ими) переменной(ыми)</b>`
    ...
    [
      `Деструктуризация`,
      `позволяет разложить обьект или массив сразу на переменные.<hr>Деструктуризация массива где мы получаем переменные a b c которые соответствуют элементам массива в том порядке в котором мы их обьявили. При этом если с в массиве не существует то по умолчанию ей присвоится значение 4<pre>
const [a, , b, c = 4] = array;</pre><hr>Деструктуризация объекта где для имен переменных мы переименовываем свойста обьекта, а также если age не задан ему будет присвоено значение 12.<pre>
const {name: newVar, age: newAge = 12} = object;</pre>`
    ],
    [
      `.target`,
...
array.sort((x, y) => {
  return x - y;
    ...
    [
      `.insertAdjacentHTML(m, t)`,
      `Вставляет разметку - t, в определенное место внутри элемента - m, при этом работает быстрее чем .innerHTML потому что не перезаписывает данные внутри. m:<br>afterbegin - сразу после открывающего тега элемента<br>afterend - после закрывающего тега<br>beforeend - перед закрывающим тегом<br>beforebegin - до начала открывающего тега`
    ],
    [
      `.offsetWidth`,
...
el.setAttribute('style', 'background: red;');</pre>`
    ...
    [
      `.search(r)`,
      `Метод строки возвращает первое вхождение поиска переданного регулярного выражения - r. Не работает с флагом g`
    ],
    [
      `.match(r)`,
      `Метод строки возвращает массив вхождений поиска переданного регулярного выражения - r`
    ],
    [
      `.test(r)`,
    ...
    [
      ...
      `Возвращает объект с вычисленными свойствами CSS (даже те что не указаны явно) элемента - e`
    ],
    [
      `.some()`,
...
}</pre>Хук useRef позволяет получить доступ к элементу на странице но только после отрисовки компонента. Необходим для работы с неконтролируемыми элементами а также сторонними ресурсами`
    ],
    [
      'useCallback()',
      `<pre>
...
&lt;Route path="about/:id" /&gt;</pre>Передача параметров в путь<pre>
app.jsx
...
function SomeComp: React.FC&lt;SomeTypeProps&gt;(props) {
  ...
    ...
    [
      'JSX разметка',
      `1) вместо class используется className<br>2) вместо атрибута for пишется htmlFor<br>3) при использовании boolean параметра можно не указывать его значение если оно равно true, а всего лишь имя переменной<br>4) события в разметке указываются как атрибуты элементов <b>onX</b> где x имя события(click, submit) а в качестве значения принимает ф-цию которая описана в логике<br>5) вместо &lt;a href="#"&gt; используется <b>&lt;link to="#"&gt;</b><br>
      <pre>
function SomeComp() {
  return (
    &lt;h1 style={{
...
};</pre>Однако такое поведение можно исправить указав соответствующий атрибут передав в него объект с компонентом. Но это опасно`
    ],
    [
