...
4) если хоть 1 значение NaN то всегда возвращается false`
    ...
    [
      ...
      <b>.match(x)</b> - вернет массив всех совпадений в строке`
    ],
    [
...
class Another extends Some {
  constructor(a, b, c) {
    ...
    [
      `Object.assign(x, y)`,
      `Возвращает модифицированный объект х с добавленными к нему св-ми из объекта у. Если в целевом объекте уже существует св-во то оно будет перезаписано из крайнего левого объекта`
    ],
    [
      `.valueOf()`,
    ...
    [
      `Массив`,
      `По сути это итерируемый объект. При обращении к элементу массива индекс сначала приводится к строке а потом получаем значение как из обычного объекта`
    ],
    [
      `Объект`,
...
obj.setter = 1;
console.log(obj.getter);</pre>Синтаксис get и set. Они связывают свойство объекта с определенной ф-цией и в последующем при обращении к этому св-ву она будет вызвана. При обращении к get вызывается ф-ция которая возвращает значение. При обращении к set вызываетс ф-ция устанавливающая новое значение переданное в аргумент в виде обычного присваивания значения`
    ],
    [
